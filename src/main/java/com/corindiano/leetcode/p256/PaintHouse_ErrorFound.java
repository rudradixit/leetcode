 ...
 */
class PaintHouse_ErrorFound {
    private static int getMin(int skip, int[] costs) {
    ...
    static int minCost(int[][] costs) {
        ...
        for (int hr=0; hr<costs.length; hr++) {
            int[] rowCosts = costs[hr];
            int min = getMin(lastChosen, rowCosts);
            sum += rowCosts[min];
            lastChosen = min;
        }
